...
function formatarUsuario(row) {
  ...
  return {
    ...
    nome:      row.nome,
    email:     row.email,
    perfil:    row.perfil, // Esperado: 'Administrativo' ou 'Líder de Produção'
    ativo:     row.ativo === 1,
...
const Usuario = {
  ...
  // Buscar usuarios por email (usado no Login)
  ...
  // Criar usuarios
  async create({ nome, email, senha, perfil = 'Administrativo' }) {
    await ready;
  ...
  // Verificar senha no momento do login
